...
public class AlphabetCodecBenchmark {
    ...
    private static final int ENCODED_INTEGER_COUNT = 10_000;
    ...
    public void setUp() {
        ...
        for (int i = 0; i < ENCODED_INTEGER_COUNT; i++) {
            this.encodedIntegers[i] = alphabetCodec.encodeIntegerAsString(this.random.nextLong(), Long.SIZE);
        }
    }

    @Benchmark
    public String benchmarkEncodeIntegerAsString() {
        return this.alphabetCodec.encodeIntegerAsString(this.random.nextLong(), Long.SIZE);
    }

    @Benchmark
    public long benchmarkDecodeStringAsInteger() {
        return this.alphabetCodec.decodeStringAsInteger(this.encodedIntegers[this.random.nextInt(this.encodedIntegers.length)], Long.SIZE);
    }
}
